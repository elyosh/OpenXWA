...
docker buildx build \
  ...
  --target artifact \
  --output type=local,dest=build/windows-package \
  .
```
...
```text
build/windows-package/openxwa-windows-x86_64-release.zip
```
...
```text
build/windows-package/openxwa-windows-x86_64-debug.zip
```
...
```text
openxwa-windows-x86_64-release/
├── OpenXWA.exe
├── *.dll
├── licenses/
├── resources/
